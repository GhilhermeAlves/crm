...
import type { Task, TaskStatus, CreateTaskRequest, UpdateTaskRequest } from "../types/task.types";
...
export const TaskService = {
  ...
  async changeStatus(companyId: string, id: string, status: TaskStatus): Promise<Task> {
    const response = await api.post<Task>(`${BASE}/${companyId}/tasks/${id}/status/${status}`);
    return response.data;
  },
