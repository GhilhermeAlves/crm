...
| 1.7.0 | 2026-08-01 | Architect | Fase Final — validação E2E em produção (VPS): resolucao via auth-service (RESOLVED/provisionamento), RBAC 200/403, auditoria via CurrentUser, observabilidade (401/404/500 padronizados), rollback por flag validado e revertido para true; corrigido healthcheck do Keycloak (rota /health na porta 9000); sem push; aguardando aprovacao |
| 1.8.0 | 2026-08-01 | Architect | Correção das pendências pré-existentes: (1) optimistic locking na auditoria — AuditLogRepositoryImpl.save() agora zera o id quando não existe (INSERT via @GeneratedValue) em vez de merge/UPDATE; auditoria validada em produção (RESET_PASSWORD/CREATE gravados); (2) register com company_id null — resolveCompanyForRegistration() valida company informada (400 se inexistente) e usa company padrão ativa quando ausente; register 201 validado; frontend envia companyId só quando presente; 57 testes backend + 35 frontend verdes; build OK |
